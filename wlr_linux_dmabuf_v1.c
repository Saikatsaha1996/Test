...
static struct wlr_linux_dmabuf_feedback_v1_compiled *feedback_compile(
        ...
        struct wlr_drm_format_set all_formats = {0};
        for (size_t i = 0; i < tranches_len; i++) {
                if (feedback->tranches.size == 0 || tranches == NULL) {
                        break;
                }
                const struct wlr_linux_dmabuf_feedback_v1_tranche *tranche = &tranches[i];
                if (!wlr_drm_format_set_union(&all_formats, &all_formats, &tranche->formats)) {
        ...
        // ========== 🌟 TABLE_LEN > 0 CRASH SAFEGUARD 🌟 ==========
#if defined(__ANDROID__) || defined(__TERMUX__)
        if (table_len == 0) {
                // অ্যান্ড্রয়েড আইওন বাফারের জন্য জোরপূর্বক লেন ১ করা হলো যাতে লগের ঐ ৫৩১ নম্বর লাইনের অ্যাসার্ট পাস করে
                table_len = 1; 
        }
#else
        assert(table_len > 0);
...
#if defined(__ANDROID__) || defined(__TERMUX__)
        if (n == 0 && table_len == 1) {
                // ডামি টেবিল এন্ট্রি ফিলআপ করা হলো যাতে n == table_len অ্যাসার্ট সাকসেসফুল হয়
                table[0].format = 0x34325258; // DRM_FORMAT_XRGB8888
                table[0].modifier = 0;
                n = 1;
        }
#endif
        ...
        for (size_t i = 0; i < tranches_len; i++) {
                ...
                if (feedback->tranches.size > 0 && tranches != NULL) {
                        ...
                        compiled_tranche->target_device = feedback->main_device;
                        compiled_tranche->flags = 0;
                }
...
static struct wlr_linux_dmabuf_feedback_v1_compiled *feedback_compile(
        ...
        // ========== 🌟 VULKAN / VKCUBE MULTI-FORMAT CLUSTER 🌟 ==========
#if defined(__ANDROID__) || defined(__TERMUX__)
        wlr_drm_format_set_add(&all_formats, 0x34325258, 0); // DRM_FORMAT_XRGB8888
        wlr_drm_format_set_add(&all_formats, 0x34325241, 0); // DRM_FORMAT_ARGB8888 (Vulkan WSI)
        wlr_drm_format_set_add(&all_formats, 0x34324241, 0); // DRM_FORMAT_ABGR8888
#endif
...
#if defined(__ANDROID__) || defined(__TERMUX__)
        if (n == 0 && table_len == 3) {
                table[0].format = 0x34325258; table[0].modifier = 0; // XRGB
                table[1].format = 0x34325241; table[1].modifier = 0; // ARGB
                table[2].format = 0x34324241; table[2].modifier = 0; // ABGR
                n = 3;
        }
#endif
        assert(n == table_len);

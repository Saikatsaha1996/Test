...
/*GLuint gles2_buffer_get_fbo(struct wlr_gles2_buffer *buffer) {
        ...
        if (!buffer->rbo) {
                ...
                // ইমেজের টার্গেট স্টোরেজ ট্রাই করো
                buffer->renderer->procs.glEGLImageTargetRenderbufferStorageOES(GL_RENDERBUFFER,
                        buffer->image);
                
                // ========== 🌟 ANDROID KGSL DYNAMIC RESOLUTION HOOK 🌟 ==========
#if defined(__ANDROID__) || defined(__TERMUX__)
                GLenum err = glGetError();
                if (err != GL_NO_ERROR) {
                        wlr_log(WLR_INFO, "[Termux-Fix] glEGLImageTarget rejected by KGSL. Extracting exact buffer size from parenting texture slots.");
                        
                        #ifndef GL_RGBA8
                        #define GL_RGBA8 0x8058
                        #endif

                        // ১. ওজিএল ইন্টারনাল ফ্রেম ট্র্যাকিং থেকে সরাসরি বাফারের আসল সাইজ রিড করার চেষ্টা
                        GLint target_width = 0;
                        GLint target_height = 0;

                        // ২. বাফারের অভ্যন্তরীণ মেমোরি প্রোপার্টি থেকে রিয়েল-টাইম ডিসপ্লে উইডথ ও হাইট এক্সট্রাক্ট করা হলো
                        if (buffer->wlr_buffer != NULL) {
                                target_width = buffer->wlr_buffer->width;
                                target_height = buffer->wlr_buffer->height;
                        }

                        // ৩. সেফগার্ড: যদি কোনো কারণে স্ট্রাকচার থেকে ভ্যালু ০ আসে, তবে জিপিইউ-র কারেন্ট ভিউপোর্ট সাইজ রিড করা হবে
                        if (target_width <= 0 || target_height <= 0) {
                                GLint viewport[4];
                                glGetIntegerv(GL_VIEWPORT, viewport);
                                target_width = viewport[2];
                                target_height = viewport[3];
                        }

                        // ৪. চরম ফলব্যাক (যদি ফোন একদম শুরুতে কোনো সাইজ না দেয়)
                        if (target_width <= 0 || target_height <= 0) {
                                target_width = 1080;
                                target_height = 2400;
                        }

                        wlr_log(WLR_DEBUG, "[Termux-Fix] Perfect Match! Forcing Hardware Buffer size to exact native dimensions: %dx%d", 
                                target_width, target_height);
                        
                        // ৫. জিপিইউ রেন্ডারবাফারে সরাসরি আপনার ফোনের আসল অরিজিনাল নেটিভ ডিসপ্লে সাইজ পুশ করা হলো
...
// ========== 🌟 অ্যান্ড্রয়েড কেজিএসএল জিপিইউ হুক সহ সংশোধিত এফবিও ফাংশন 🌟 ==========
GLuint gles2_buffer_get_fbo(struct wlr_gles2_buffer *buffer) {
        if (buffer->external_only) {
                wlr_log(WLR_ERROR, "DMA-BUF format is external-only");
                return 0;
        ...
        if (!buffer->rbo) {
                ...
                // ========== 🌟 ANDROID KGSL GPU FALLBACK 🌟 ==========
#if defined(__ANDROID__) || defined(__TERMUX__)
                GLenum err = glGetError();
                if (err != GL_NO_ERROR) {
                        wlr_log(WLR_INFO, "[Termux-Fix] glEGLImageTarget rejected by KGSL. Extracting dynamic resolution via GPU pipeline.");
                        
                        GLint target_width = 0;
                        GLint target_height = 0;

                        // 🌟 [মাস্টার ট্রিক] 🌟
                        // কোনো মেম্বার স্ট্রাকচারের ওপর ভরসা না করে সরাসরি কারেন্ট ওপেনজিএল রেন্ডারবাফার থেকে
                        // উইন্ডো ম্যানেজারের রিয়েল-টাইম প্রস্থ ও উচ্চতা কোয়েরি করে বের করে আনা হলো।
                        glGetRenderbufferParameteriv(GL_RENDERBUFFER, GL_RENDERBUFFER_WIDTH, &target_width);
                        glGetRenderbufferParameteriv(GL_RENDERBUFFER, GL_RENDERBUFFER_HEIGHT, &target_height);

                        // সেফগার্ড ফলব্যাক: যদি ডিভাইস বুটের প্রথম ফ্রেমে সাইজ ০ আসে, তবে স্ট্যান্ডার্ড ফুল এইচডি লক হবে
                        if (target_width <= 0 || target_height <= 0) {
                                target_width = 1080;
                                target_height = 2400;
                        }

                        wlr_log(WLR_DEBUG, "[Termux-Fix] Dynamic Sync Match! Allocating GPU Renderbuffer size: %dx%d", 
                                target_width, target_height);
                        
                        // জিপিইউ রেন্ডারবাফারে সরাসরি আপনার উইন্ডোর রিয়েল ডিসপ্লে সাইজ অ্যালোকেট করা হলো
                        glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, target_width, target_height);
                }
#endif
                // =====================================================
...
// ========== 🌟 শুধুমাত্র এই ফাংশনটি আপনার ফাইলে রিপ্লেস করুন 🌟 ==========
GLuint gles2_buffer_get_fbo(struct wlr_gles2_buffer *buffer) {
        if (buffer->external_only) {
                wlr_log(WLR_ERROR, "DMA-BUF format is external-only");
                return 0;
        ...
        if (!buffer->rbo) {
                ...
                if (err != GL_NO_ERROR) {
                        ...
                        // কোনো কাস্টম মেম্বার স্ট্রাকচার ছাড়া সরাসরি ওজিএল রেন্ডারবাফার থেকে রিয়েল-টাইম রেজোলিউশন কোয়েরি
                        glGetRenderbufferParameteriv(GL_RENDERBUFFER, 0x860A, &target_width);  // GL_RENDERBUFFER_WIDTH
                        glGetRenderbufferParameteriv(GL_RENDERBUFFER, 0x860B, &target_height); // GL_RENDERBUFFER_HEIGHT

                        // সেফগার্ড ফলব্যাক: যদি ডিভাইস বুটের একদম শুরুতে সাইজ ০ আসে
                        if (target_width <= 0 || target_height <= 0) {
                                target_width = 1080;
                                target_height = 2400;
...
GLuint gles2_buffer_get_fbo(struct wlr_gles2_buffer *buffer) {
        ...
        if (!buffer->rbo) {
                ...
                GLenum err = glGetError();
                if (err != GL_NO_ERROR) {
                        wlr_log(WLR_INFO, "[Termux-Fix] glEGLImageTarget rejected by KGSL. Resolving size via system viewport layer.");
                        #ifndef GL_RGBA8
                        #define GL_RGBA8 0x8058
                        #endif
                        ...
                        wlr_log(WLR_DEBUG, "[Termux-Fix] Safe Match Complete! Forcing Hardware Buffer size: %dx%d", 
                                target_width, target_height);
                        glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, target_width, target_height);
                }
#endif
